...
$(deriveLiftMany [''URIAuth, ''UserId, ''URI, ''Peek_Report, ''Order, ''JSONText, ''Permissions, ''CIString, ''ImageFile, ''File, ''FileSource, ''ImageCrop, ''ImageType, ''Markup, ''Pandoc, ''Meta, ''MetaValue, ''Inline, ''QuoteType, ''Text.Pandoc.MathType, ''Format, ''Citation, ''CitationMode, ''Block, ''ListNumberStyle, ''ListNumberDelim, ''Alignment])
