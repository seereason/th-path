...
toLensDecs' v gkey = do
  ptyp <- pathType (pure (bestType gkey)) v
  tlc <- execWriterT $ toLensClauses v gkey
  when (not (null tlc)) $
       (runQ $ sequence
