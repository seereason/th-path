...
import Language.Haskell.TH.Path.Decs.IsPath (isPathNodeDecs)
import Language.Haskell.TH.Path.Decs.PathsOf (isPathDecs)
...
pathDecs = execWriterT $ allPathStarts >>= mapM_ doNode
...
doNode v = do
  ...
  pathKeys v >>= Set.mapM_ (isPathDecs v)
  -- generate the lenses
