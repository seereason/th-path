...
module Language.Haskell.TH.Path.ATP
    ( unfoldApply
    , expandBindings
    ) where
...
import Data.Logic.ATP.Unif (Unify(UTermOf, unify'))
...
instance HasFixity Type where
    precedence _ = error "HasFixity Type precedence"
    associativity _ = error "HasFixity Type associativity"
...
instance HasFixity Context where
    precedence _ = error "HasFixity Context precedence"
    associativity _ = error "HasFixity Context associativity"
